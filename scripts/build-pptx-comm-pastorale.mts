/* ============================================================================
   Génère un PowerPoint (.pptx) PROPRE pour le séminaire
   « Le numérique au service de la communication éducative et pastorale » (SENEC).

   Pourquoi ce script : l'ancien support.pptx (deck d'origine) était mal agencé
   et par endroits illisible (zones de texte qui se chevauchent, débordements).
   Ici on régénère le deck à partir du CONTENU STRUCTURÉ déjà maintenu dans le
   code (lib/seminaires/communication-pastorale.ts), avec un moteur de mise en
   page séquentiel : chaque bloc occupe une région bornée et NON chevauchante,
   et tout texte est en `fit: "shrink"` → aucun débordement possible.

   Exécution :  npx tsx scripts/build-pptx-comm-pastorale.mts
   Sortie    :  public/seminaires/communication-pastorale/support.pptx
   ========================================================================== */
...
const C = {
  ink: "1B2A22", // texte courant sur fond clair
  green: "176B45", // vert de marque
  greenDark: "0E2A1E", // fond couverture / clôture (vert encre profond)
  green700: "1F7A50",
  gold: "D99A1E",
  goldSoft: "F7EFD9",
  gray: "5A6660",
  cream: "F6F2EA", // fond des diapositives standard (comme la version web)
  white: "FFFFFF",
  cardFill: "FFFFFF",
  cardLine: "E3E7E3",
};

/* Teintes des encadrés « highlight » selon le ton. */
const TONE: Record<string, { fill: string; bar: string; label: string }> = {
  info: { fill: "E9EEFA", bar: "1E40AF", label: "Repère" },
  pastoral: { fill: "F0EAFA", bar: "7C4DBC", label: "Repère pastoral" },
  warning: { fill: "FBF1DA", bar: "D99A1E", label: "Attention" },
  success: { fill: "E6F2EB", bar: "176B45", label: "Méthode" },
  default: { fill: "E6F2EB", bar: "176B45", label: "Repère" },
};
...
const PAGE_W = 13.333;
const PAGE_H = 7.5;
const MX = 0.62; // marge gauche/droite
...
pptx.subject = COMMUNICATION_PASTORALE.meta.title;
pptx.title = COMMUNICATION_PASTORALE.meta.title;

const TOTAL = COMMUNICATION_PASTORALE.slides.length;
...
/** Bandeau d'en-tête commun aux diapositives standard. Renvoie la région de contenu. */
function addHeader(slide: Slide, s: CommSlide): Region {
  // Filets décoratifs en haut (vert + segment doré).
  slide.addShape(pptx.ShapeType.rect, { x: 0, y: 0, w: PAGE_W, h: 0.12, fill: { color: C.green } });
  slide.addShape(pptx.ShapeType.rect, { x: 0, y: 0, w: 3.2, h: 0.12, fill: { color: C.gold } });
  ...
  // Filet doré court sous l'en-tête.
  slide.addShape(pptx.ShapeType.rect, { x: MX, y: headerBottom + 0.04, w: 1.5, h: 0.045, fill: { color: C.gold } });
...
/** Pied : encadré « repère » (footer), pastille de page, étiquette SENEC. */
function addFooter(slide: Slide, s: CommSlide) {
  if (s.footer) {
    ...
    slide.addShape(pptx.ShapeType.rect, { x: MX, y: 6.72, w: 0.07, h: 0.5, fill: { color: C.gold } });
  ...
  slide.addText(`SENEC · ${COMMUNICATION_PASTORALE.meta.referenceDate}`, {
...
/** Poids relatif d'un bloc pour le partage vertical de la région de contenu. */
function blockWeight(b: CommSlideBlock): number {
  switch (b.kind) {
    ...
    default:
      return 2.4; // pillars, principles, steps, channels, publics, listes
  }
}

/** Découpe une région verticalement selon les poids des blocs et rend chacun. */
...
/* ===========================================================================
   Rendu des cartes / grilles
   ======================================================================== */

/** Grille de cartes (label + titre + description). */
function cardGrid(
  slide: Slide,
  ...
  items.forEach((it, idx) => {
    ...
    // Bande d'accent à gauche de la carte.
    slide.addShape(pptx.ShapeType.rect, { x, y, w: 0.08, h: ch, fill: { color: C.green } });
    ...
    if (opts.bigLetter) {
      // Lettre/numéro en grand (style principe).
...
function renderBlock(slide: Slide, b: CommSlideBlock, region: Region) {
  switch (b.kind) {
    ...
    case "highlight": {
      ...
      slide.addShape(pptx.ShapeType.rect, { x: region.x, y: region.y, w: 0.08, h: region.h, fill: { color: t.bar } });
      slide.addText(b.text, {
    ...
    case "channels": {
      ...
      b.items.forEach((it, idx) => {
        ...
        slide.addShape(pptx.ShapeType.rect, { x, y, w: 0.07, h: ch, fill: { color: C.gold } });
        slide.addText(
          [
    ...
    case "publics": {
      ...
      slide.addTable([header, ...body], {
        ...
        colW: [1.7, 2.2, 2.85, 2.7, 2.68],
        fontFace: FONT, fontSize: 10.5, border: { type: "solid", color: "D7DED8", pt: 1 },
...
/* ===========================================================================
   Diapositives spéciales (couverture & clôture)
   ======================================================================== */

function addCoverSlide(slide: Slide, s: CommSlide) {
  slide.background = { color: C.greenDark };
  // Accents géométriques discrets.
  slide.addShape(pptx.ShapeType.rect, { x: 0, y: 0, w: PAGE_W, h: 0.16, fill: { color: C.gold } });
  slide.addShape(pptx.ShapeType.rect, { x: 0, y: PAGE_H - 0.16, w: PAGE_W, h: 0.16, fill: { color: C.green } });
  ...
  slide.addShape(pptx.ShapeType.rect, { x: MX, y: 3.5, w: 2.4, h: 0.07, fill: { color: C.gold } });
  if (s.subtitle) {
    slide.addText(s.subtitle, {
  ...
  // Les trois passages, en chips bien espacées (correction du chevauchement d'origine).
  ...
  if (pillarsBlock && pillarsBlock.kind === "pillars") {
    ...
    items.forEach((it, i) => {
      ...
      slide.addText(it.label, {
        x: x + 0.22, y: y + 0.16, w: 0.7, h: 0.5, fontFace: FONT, fontSize: 24, bold: true, color: C.gold,
...
function addClosingSlide(slide: Slide, s: CommSlide) {
  ...
  slide.addShape(pptx.ShapeType.rect, { x: MX, y: 2.78, w: 2.4, h: 0.06, fill: { color: C.gold } });

  // Paragraphes (objectif + message) en clair.
  const paras = s.blocks.filter((b): b is Extract<CommSlideBlock, { kind: "paragraph" }> => b.kind === "paragraph");
  let cy = 3.1;
  ...
  // Les quatre C (flow) en chips dorées.
...
async function main() {
  for (const s of COMMUNICATION_PASTORALE.slides) {
  ...
  console.log(`✓ PPTX généré (${TOTAL} diapositives) → ${outPath}`);
}
