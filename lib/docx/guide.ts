...
export async function buildGuideDocx(roleKey: string): Promise<Buffer> {
  const raw = GUIDES[roleKey];
  if (!raw) throw new Error(`Guide introuvable : ${roleKey}`);
  // Contenu dérivé : listes système toujours à jour (formations, rôles,
  // supports) injectées avant génération — voir lib/guides/dynamic-facts.
  const guide = prepareGuide(raw);
  ...
  const front = [
    ...
    bodyText("À l'issue de ce guide, vous serez capable de :", {
      italic: true,
    }),
    ...guide.objectives.map((o, i) => numberedItem(o, `${i + 1}.`)),
    spacer(160),
    heading1("Prérequis"),
    bodyText("Avant de commencer la formation, vérifiez que :", {
      italic: true,
    }),
    ...
    c.sections.forEach((s) => {
      ...
      if (s.steps && s.steps.length > 0) {
        s.steps.forEach((st, i) => {
          ...
          if (st.warning)
            body.push(calloutParagraph("Attention", st.warning, "warning"));
        });
      }
      ...
      if (s.caveat)
        body.push(calloutParagraph("Mise en garde", s.caveat, "warning"));
    });
  });
  ...
  bodyText(
    "Réponses aux questions fréquemment posées par les utilisateurs ayant ce rôle.",
    { italic: true },
  );
  ...
  bodyText("Termes spécifiques au rôle utilisés dans ce guide.", {
    italic: true,
  });
